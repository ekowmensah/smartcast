...
class VoteController extends BaseController
{
    ...
    public function vote()
    {
        try {
            ...
            try {
                ...
                // Generate receipt only for non-mobile money payments
                // Mobile money payments have receipts generated by PaymentService to avoid duplicates
                $receipt = null;
                if (!$this->isPaymentProcessedByPaymentService($transactionId)) {
                    try {
                        $receipt = $this->receiptModel->generateReceipt($transactionId);
                        error_log("API: Receipt generated for transaction {$transactionId}: " . $receipt['short_code']);
                    } catch (\Exception $e) {
                        error_log("API: Receipt generation failed for transaction {$transactionId}: " . $e->getMessage());
                        // Don't fail the entire process if receipt generation fails
                    }
                } else {
                    error_log("API: Skipping receipt generation - handled by PaymentService for transaction: {$transactionId}");
                }
     ...
     */
    private function isPaymentProcessedByPaymentService($transactionId)
    {
        // Get the transaction to check its provider_reference
        try {
            ...
            error_log("API: Error checking payment service processing: " . $e->getMessage());
