...
class VoteController extends BaseController
{
    ...
    private function processSuccessfulPayment($transaction, $paymentStatus)
    {
        try {
            ...
            // Generate receipt only for non-mobile money payments (e.g., simulations)
            // Mobile money payments have receipts generated by PaymentService to avoid duplicates
            $receipt = null;
            if (!$this->isPaymentProcessedByPaymentService($transaction)) {
                try {
                    error_log("Generating receipt for non-mobile money transaction: " . $transaction['id']);
                    $voteReceiptModel = new \SmartCast\Models\VoteReceipt();
                    $receipt = $voteReceiptModel->generateReceipt($transaction['id']);
                    error_log("Receipt generated successfully: " . $receipt['short_code']);
                } catch (\Exception $e) {
                    error_log("Receipt generation failed: " . $e->getMessage());
                    // Don't throw the exception - continue without receipt
                }
            } else {
                error_log("Skipping receipt generation - handled by PaymentService for transaction: " . $transaction['id']);
            }
    ...
    /**
     * Check if payment was processed by PaymentService (mobile money)
     * This helps avoid duplicate receipt generation
     */
    private function isPaymentProcessedByPaymentService($transaction)
    {
        // Check if there's a corresponding payment_transactions record
        // Mobile money payments create records in both tables
        try {
            ...
            $paymentTransaction = $db->selectOne(
                ...
                ['ref' => $transaction['provider_reference'], 'ref2' => $transaction['provider_reference']]
            );
            
            return $paymentTransaction !== null;
        } catch (\Exception $e) {
            error_log("Error checking payment service processing: " . $e->getMessage());
            return false; // Default to generating receipt if unsure
        }
    }
